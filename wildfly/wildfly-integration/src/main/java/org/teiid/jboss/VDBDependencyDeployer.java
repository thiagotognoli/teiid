...
class VDBDependencyDeployer implements DeploymentUnitProcessor {
    ...
    public void deploy(DeploymentPhaseContext phaseContext) throws DeploymentUnitProcessingException {
        ...
        ArrayList<ModuleDependency> localDependencies = new ArrayList<>();
        ArrayList<ModuleDependency> userDependencies = new ArrayList<>();
        ...
        if (moduleNames != null) {
            ...
            while (modules.hasMoreTokens()) {
                ...
                try {
                    ...
                    localDependencies.add(ModuleDependency.Builder.of(callerModuleLoader, moduleName).setOptional(false).setExport(false).setImportServices(false).setUserSpecified(false).build());
                } catch (ModuleLoadException e) {
                    // this is to handle JAR based deployments which take on name like "deployment.<jar-name>"
                    try {
                        sml.loadModule(moduleName);
                        userDependencies.add(ModuleDependency.Builder.of(sml, moduleName).setOptional(false).setExport(false).setImportServices(false).setUserSpecified(true).build());
